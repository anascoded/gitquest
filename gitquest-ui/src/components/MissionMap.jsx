        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 500, color: '#c8daf0', fontFamily: 'monospace' }}>{level.title}</div>
          ...
          <div style={{ height: 4, background: '#1a2a45', borderRadius: 99, overflow: 'hidden', marginBottom: '1.25rem' }}>
            <div style={{ height: '100%', width: `${total > 0 ? (completed / total) * 100 : 0}%`, background: color, borderRadius: 99, transition: 'width 0.4s' }} />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: '1.5rem' }}>
            {missions.map((mission, idx) => {
              ...
              return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 12, fontFamily: 'monospace', minWidth: 16, color: done ? color : isNext ? color + '99' : '#2a3a55' }}>
                    {done ? '✓' : locked ? '🔒' : isNext ? '▶' : '○'}
                  </span>
                      <span style={{ fontSize: 12, color: done ? '#00cc66' : locked ? '#2a3a55' : '#c8daf0', fontFamily: 'monospace' }}>
                    {mission.title}
                  </span>
                    </div>
  ...
  return (
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid #1a2a45', background: '#080c17' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span style={{ fontSize: 10, color: '#4a6fa5', letterSpacing: '0.08em', fontFamily: 'monospace' }}>MISSION PROGRESS</span>
              <span style={{ fontSize: 10, color: '#00ff88', fontFamily: 'monospace' }}>{totalCompleted}/{totalMissions}</span>
            </div>
            <div style={{ height: 5, background: '#1a2a45', borderRadius: 99, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${totalMissions > 0 ? (totalCompleted / totalMissions) * 100 : 0}%`, background: '#00ff88', borderRadius: 99, transition: 'width 0.5s' }} />
            </div>
          </div>
        ...
        <div style={{ flex: 1, padding: '1.5rem', position: 'relative' }}>
          <div style={{ fontSize: 10, color: '#4a6fa5', letterSpacing: '0.12em', marginBottom: '1.5rem', fontFamily: 'monospace' }}>
          ...
          <div style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button
                onClick={onBack}
                style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#e24b4a', background: '#0d1526', border: '1px solid #e24b4a44', borderRadius: 99, padding: '7px 16px', cursor: 'pointer', fontFamily: 'monospace' }}
            >
              ✕ Abort Mission
            </button>
            <button
                onClick={() => setShowCredits(true)}
                style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#4a6fa5', background: '#0d1526', border: '1px solid #1a2a45', borderRadius: 99, padding: '7px 16px', cursor: 'pointer', fontFamily: 'monospace' }}
            >
              ℹ Credits
            </button>
            {showCredits && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    <button
                        ...
                        style={{ position: 'absolute', top: 14, right: 16, background: 'none', border: 'none', color: '#4a6fa5', fontSize: 18, cursor: 'pointer', lineHeight: 1 }}
                    >✕</button>

                    {/* Title */}
                    <div style={{ fontSize: 10, color: '#4a6fa5', letterSpacing: '0.14em', marginBottom: 6 }}>ABOUT</div>
                    ...
                    <div style={{ fontSize: 12, color: '#4a6fa5', marginBottom: '1.5rem', lineHeight: 1.7 }}>
                      An interactive web app designed to teach Git through immersive, story-driven missions.
                      Players take on the role of a cyber agent battling Shadow Breach — learning real Git
                      commands through hands-on field scenarios, progressive levels, and a live leaderboard.
                    </div>
                    ...
                    {/* Tech stack */}
                    <div style={{ fontSize: 10, color: '#4a6fa5', letterSpacing: '0.14em', marginBottom: 10 }}>BUILT WITH</div>
            ...
            {tech}
                    ...
                    {/* Team */}
                    <div style={{ fontSize: 10, color: '#4a6fa5', letterSpacing: '0.14em', marginBottom: 10 }}>AGENTS</div>
            ...
            <span style={{ fontSize: 10, color: '#2a3a55', minWidth: 20, fontFamily: 'monospace' }}>
              0{i + 1}
                    ...
                    {/* Copyright */}
                    <div style={{ fontSize: 11, color: '#2a3a55', textAlign: 'center', letterSpacing: '0.06em' }}>
              ...
              return (
                  <g key={level._id}
                    ...
                    {(isActive || isFirst) && !isComplete && (
                        <rect x={pos.x - 4} y={pos.y - 4} width={NODE_SIZE + 8} height={NODE_SIZE + 8} rx="14"
                    ...
                    <text x={pos.x + NODE_SIZE / 2} y={pos.y + 46}
                          textAnchor="middle" fontSize="11" fill={textColor} fontFamily="monospace" fontWeight="bold">
